...
public class TestsErrorGenerator {

    @TestFactory
    public Stream<DynamicTest> generateTests() throws URISyntaxException, IOException {
        URL tests = TestsErrorGenerator.class.getClassLoader().getResource("error");
        Objects.requireNonNull(tests, "No error dir in resources");

        Path testDir = Paths.get(tests.toURI());
        return Files.walk(testDir, FileVisitOption.FOLLOW_LINKS)
                .filter(Files::isDirectory)
                .filter(TestsErrorGenerator::hasJavaFiles)
